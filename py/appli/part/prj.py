...
def GlobalTaxoCompute():
    ...
    samples = database.GetAll("""select psampleid, daterecalculhistotaxo,pp.instrumtype
                ...
                and (
                      exists (select 1 from obj_head oh 
                              join acquisitions a on oh.acquisid = a.acquisid 
                              where a.acq_sample_id=ps.sampleid and oh.classif_when>ps.daterecalculhistotaxo)
