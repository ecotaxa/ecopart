...
def indexPart():
    ...
    g.headcenter = """<h1 style='text-align: center;cursor: pointer;' >
      <span onclick="$('#particleinfodiv').toggle()"><b>PARTICLE</b> module 
      <span class='glyphicon glyphicon-info-sign'></span> </span> 
      <a href='https://ecotaxa.obs-vlfr.fr' style='font-size:medium;margin-left: 50px;'>Go to Ecotaxa</a></h2>"""
    g.useselect4 = True
...
def GetSQLVisibility():
    ...
    if current_user.has_role(database.AdministratorLabel):
        ...
        if current_user.is_authenticated:
            sqlvisible += """ 
                    when ppriv.member is not null 
                    and oldestsampledate+make_interval(0,public_visibility_deferral_month)<=current_date then 
...
def GetFilteredSamples(Filter=None, GetVisibleOnly=False, ForceVerticalIfNotSpecified=False,
    ...
    if Filter.get("MapN", '') != "" and Filter.get("MapW", '') != "" and Filter.get("MapE", '') != "" \
            and Filter.get("MapS", '') != "":
...
def PartstatsampleGetData():
    ...
    data['partprojcount'] = database.GetAll("""SELECT pp.ptitle,count(*) nbr
        ...
        group by pp.ptitle,pp.do_email,do_name,qpp.email,qpp.name,p.visible,pp.instrumtype,pp.pprojid,visibility
    ...
    taxo_stat_by_proj = {}
    for r in taxo_stat:
        result_taxo_stat['nbrobj'] += r['nbr']
        result_taxo_stat['nbrobjval'] += r['nbrval']
        if r['pprojid'] not in taxo_stat_by_proj:
            taxo_stat_by_proj[r['pprojid']] = {'nbrobj': 0, 'nbrobjval': 0, 'pctobjval': 0}
        taxo_stat_by_proj[r['pprojid']]['nbrobj'] += r['nbr']
        taxo_stat_by_proj[r['pprojid']]['nbrobjval'] += r['nbrval']
        if r['nbr'] == r['nbrval']:
            result_taxo_stat['pctval100pct'] += 1
        if r['nbr'] > r['nbrval']:
            result_taxo_stat['pctpartval'] += 1
    if len(taxo_stat) > 0:
        result_taxo_stat['pctval100pct'] = round(100 * result_taxo_stat['pctval100pct'] / len(taxo_stat), 1)
        result_taxo_stat['pctpartval'] = round(100 * result_taxo_stat['pctpartval'] / len(taxo_stat), 1)
    if result_taxo_stat['nbrobj'] > 0:
        result_taxo_stat['pctobjval'] = round(100 * result_taxo_stat['nbrobjval'] / result_taxo_stat['nbrobj'], 1)
    data['taxostat'] = result_taxo_stat
    data['taxostatbyproj'] = {}
    for k, r in taxo_stat_by_proj.items():
        if r['nbrobj']:
            data['taxostatbyproj'][k] = round(100 * r['nbrobjval'] / r['nbrobj'], 1)
    print(data['taxostatbyproj'])

    data['depthhisto'] = database.GetAll("""
            SELECT coalesce(case when bottomdepth<500 then '0- 500' 
                                 else trunc(bottomdepth,-3)||'-'||(trunc(bottomdepth,-3)+1000) end
                            ,'Not defined') slice
      , count(*) nbr
