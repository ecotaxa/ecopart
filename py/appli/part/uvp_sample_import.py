...
def CreateOrUpdateSample(pprojid, headerdata):
    ...
    sample = db.session.query(partdatabase.part_samples).filter_by(profileid=headerdata['profileid'],
                                                                   pprojid=pprojid).first()
    if sample is None:
    ...
    # false car dans les fichiers UVP historique c'est la notation degree.minute
    sample.latitude = ConvTextDegreeToDecimalDegree(headerdata['latitude'], False)
    sample.longitude = ConvTextDegreeToDecimalDegree(headerdata['longitude'], False)
    # Nouvelle colonne optionnel, par défaut organisé par (P)ression
            ...
            if rowpart[1].find("OVER_EXPOSED") >= 0:
                ...
                if organizedbydepth:
                    ...
                    if integrationtime <= 0:
                        raise Exception(
                            f"GenerateRawHistogramUVPAPP: Sample {UvpSample.psampleid} : integrationtime must be a "
                            "positive value for horizontal profile")
                    dateheure = datetime.datetime.strptime(dateheuretxt, "%Y%m%d-%H%M%S")
                ...
                if flash in ('0', '1'):
                    if partition not in segmented_data[flash]:
                        if organizedbydepth:
                            segmented_data[flash][partition] = {'depth': partition, 'imgcount': 0, 'area': {},
                                                                'timestamp': 0}  # 'time':dateheuretxt,
                        else:
                            segmented_data[flash][partition] = {'depth': 0, 'time': partition, 'imgcount': 0,
                                                                'area': {}}
...
def GenerateRawHistogram(psampleid):
    ...
    # le changement de format n'as pas d'impact car seule les 3 premières colonnes sont utilisées et c'est le format
    # de la 3eme qui change dans l'instrument on ne se sert que de la première valeurs donc le découpage suivant *
    # et la prise de la première valeur donne le même résultat.
    server_root = Path(app.config['SERVERLOADAREA'])
    dossier_uvp_path = server_root / prj.rawfolder
...
def GenerateParticleHistogram(psampleid):
    """
    Génération de l'histogramme particulaire détaillé (45 classes) et réduit (15 classes) à partir des fichiers RAW
    :param psampleid:
    :return:
    """
    uvp_sample = db.session.query(partdatabase.part_samples).filter_by(psampleid=psampleid).first()
    ...
    if uvp_sample.organizedbydeepth:
        ...
        # , y compris celle qui n'existent (bincount génère touts les pas entre 0 et la MaxTrancheId
