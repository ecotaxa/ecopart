...
from .urls import BACKEND_API_HOST, BACKEND_API_PORT
...
class EcoTaxaInstance(object):
    ...
    @staticmethod
    def _get_url():
        return BACKEND_API_HOST + ":" + BACKEND_API_PORT[0] + "/api"
    ...
    def get_taxo2(self, classif_ids: List[int]) -> List[Dict[str, Any]]:
        ...
        """
        res = self.query_taxa_set(classif_ids)
        return [{"classif_id": r.id,
                 "nom": r.display_name,
                 "tree": self.lt_tree(r.lineage)}
                for r in res]
    ...
    def get_objects_for_sample(self, projid: int, sampleid: int, cols: List[str], only_validated: bool):
        ...
        filters = ProjectFilters(samples=str(sampleid))
        if only_validated:
    ...
    def search_objects_validated_after(self, projid: int, sampleid: Optional[int], part_sample_date: datetime):
        ...
        """
        oba = ObjectsApi(self._get_client())
        filters = ProjectFilters(validfromdate=part_sample_date.strftime("%Y-%m-%d %H:%M"))
...
def log_in_ecotaxa(user: str, password: str) -> Optional[str]:
    ...
    """
    cli = ApiClient()
    login_req = LoginReq(username=user, password=password)
    cli.configuration.host = EcoTaxaInstance._get_url()
