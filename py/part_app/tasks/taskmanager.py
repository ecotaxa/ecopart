...
from ..app import part_app, db, temp_dir_for_task
from ..db_utils import GetAll
from ..http_utils import gvg, gvp
from ..remote import EcoTaxaInstance, ECOTAXA_COOKIE
...
class AsyncTask:
    ...
    def GetWorkingDir(self):
        return temp_dir_for_task(int(self.task.id))
    ...
    def StartTask(self, param=None, step=1, FileToSave=None, FileToSaveFileName=None):
        ...
        if part_app.PythonExecutable=="PYTESTDRIVEN":
            return # Lancé de façon synchrone par un script pytest
        runtask_py = os.path.normpath(os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../runtask.py"))
        cmdfull = part_app.PythonExecutable + " " + runtask_py + " " + str(self.task.id)
...
def TaskCreateRouter(ClassName):
    ...
    # Get the cookie from HTTP client
    task.cookie = request.cookies.get(ECOTAXA_COOKIE)
