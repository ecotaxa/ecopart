...
def GetTicks(max_val):
    if np.isnan(max_val):
        max_val = 100  # Arbitraire si MaxVal n'est pas valide
    if max_val < 1:
        max_val = 1
...
def ImportCTD(psampleid, user_name, user_email):
    ...
    if prj.instrumtype == 'uvp6remote':
        ...
        if not ctd_file.exists():
            part_app.logger.info("CTD file %s missing", ctd_file.as_posix())
            return False
        part_app.logger.info("Import CTD file %s", ctd_file.as_posix())
